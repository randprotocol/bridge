...
info "network $network${TRON_RPC_URL:+ (rpc $TRON_RPC_URL)}"
npx tronbox compile
...
hex="$(grep -Eo '\(hex\) 41[0-9a-fA-F]{40}' "$log" | tail -1 | awk '{print $2}')"
base58="$(grep -Eo '\(base58\) T[1-9A-HJ-NP-Za-km-z]{33}' "$log" | tail -1 | awk '{print $2}')"
rm -f "$log"
[[ -n "$hex" ]] || die "could not find the deployed address in tronbox's output; check tron/build/"
evm_form="0x${hex:2}"
