...
`crates/shrugg-core/src/bridge/vectors.json`). It is a valid Ethereum-to-Rand transfer of 1 USDT
...
| `[454:486)` | `1111111111111111111111111111111111111111111111111111111111111111` | `to` | Rand recipient, raw 32-byte address `0x11…11` |
...
Net credited to `to` on Rand: `amount - fee = 100000000 - 1000 = 99999000` (8-decimal units,
i.e. 0.99999000).
...
verifier accepts this attestation and (on Rand, via `BridgeAttest`) credits `99999000` units of
the asset `blake3("shrugg-bridge-asset" || 0x0002 || token_address)` to `to`.
